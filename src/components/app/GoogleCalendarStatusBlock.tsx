...
interface GoogleCalendarStatusBlockProps {
  variant: "home" | "agenda";
  /**
   * La pantalla ya lista los conflictos y deja decidirlos ahí mismo. En ese
   * caso este bloque no vuelve a titularlos ni manda a Configuración: repetir
   * el mismo aviso dos veces en la misma pantalla no agrega información y
   * empuja la agenda hacia abajo. El detalle sigue estando en «Detalles de
   * sincronización».
   */
  conflictsHandledInPage?: boolean;
}
...
export const GoogleCalendarStatusBlock =
  component$<GoogleCalendarStatusBlockProps>(
    ({ variant, conflictsHandledInPage }) => {
      ...
      const view = useComputed$(() => {
        ...
        // Con los conflictos resueltos en la propia pantalla, el resto del
        // estado se calcula como si no los hubiera: así el bloque informa la
        // sincronización y no compite con la lista de decisiones.
        return googleCalendarOperationalView(
          conflictsHandledInPage ? { ...status, conflictCount: 0 } : status,
        );
      });
      ...
      const hasConflicts =
        !conflictsHandledInPage && (state.status?.conflictCount ?? 0) > 0;
            ...
            <details class="calendar-status-disclosure">
              ...
              <dl class="calendar-operational-details">
                ...
                <div>
                  ...
                  <dd>
                    {formatSuccessfulReview(
                      state.status.lastSuccessfulReviewAt,
                    )}
                  </dd>
                </div>
                <div>
