-- Contact import is an ancillary Coexistence stream. If Meta definitively
-- rejects every retry before accepting an app-state request, preserve that
-- failure but do not disable an otherwise verified and subscribed account.
-- Live inbound messaging and a previously authorized history delivery remain
-- usable; outbound sending stays globally paused.
